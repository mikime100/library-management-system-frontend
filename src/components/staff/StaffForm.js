...
export default function StaffForm({
  ...
  const onSubmit = async (data) => {
    try {
      ...
      toast.success(
        isEditing
          ? `Staff member "${data.username}" updated successfully!`
          : `Staff member "${data.username}" added successfully!`
      );
      if (onSuccess) {
        onSuccess();
      } else {
  ...
  return _jsxs("form", {
    ...
    children: [
      errors.root?.message &&
        _jsx("div", {
          className:
            "mb-4 text-red-600 bg-red-50 border border-red-200 rounded px-4 py-2",
          ...
          _jsxs("div", {
            children: [
              ...
              _jsxs("select", {
                ...
                children: [
                  _jsx("option", { value: "librarian", children: "Librarian" }),
