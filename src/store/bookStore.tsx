...
interface BookStore {
  ...
  addBook: (
    book: Omit<Book, "id" | "addedDate" | "updatedAt" | "availableCopies">
  ) => Promise<{ success: boolean }>;
  updateBook: (
    id: string,
    updates: Partial<Omit<Book, "id" | "addedDate" | "updatedAt">>
  ) => Promise<{ success: boolean }>;
  deleteBook: (id: string) => Promise<{ success: boolean }>;
  updateBookCopies: (bookId: string, change: number) => void;
  setFilters: (filters: Partial<BookStore["filters"]>) => void;
  setSort: (
    sortBy: BookStore["sortBy"],
    sortOrder?: BookStore["sortOrder"]
  ) => void;
      ...
      const mockBooks: Book[] = [
        {
          ...
          description:
            "A story of decadence and excess, The Great Gatsby explores themes of decadence, idealism, resistance to change, social upheaval, and excess, creating a portrait of the Roaring Twenties that has been described as a cautionary tale regarding the American Dream.",
          pageCount: 180,
          language: "English",
        ...
        },
        {
          ...
          description:
            "1984 is a dystopian social science fiction novel and cautionary tale. Thematically, it centres on the consequences of totalitarianism, mass surveillance, and repressive regimentation of people and behaviours within society.",
          pageCount: 328,
          language: "English",
        ...
        book.id === bookId
          ? {
              ...
              availableCopies: Math.max(
                0,
                Math.min(book.availableCopies + change, book.totalCopies)
              ),
              updatedAt: new Date().toISOString(),
            }
          : book
