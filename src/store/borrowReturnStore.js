...
export const useBorrowReturnStore = create((set, get) => ({
    borrowRecords: [],
    currentBorrows: [],
    overdueBorrows: [],
    selectedRecord: null,
    ...
    fetchBorrowRecords: async () => {
        ...
        try {
            ...
            const borrowRecords = res.data.map((r) => {
                const now = new Date();
            ...
            const now = new Date();
            const currentBorrows = filteredRecords.filter((record) => !record.returnDate && new Date(record.dueDate) >= now);
            const overdueBorrows = filteredRecords.filter((record) => !record.returnDate && new Date(record.dueDate) < now);
            set({
    ...
    renewBook: async (borrowId, newDueDate) => {
        ...
        try {
            ...
            set((state) => {
                const updatedRecords = state.borrowRecords.map((record) => record.id === borrowId
                    ? {
                        ...record,
                        dueDate: newDueDate,
                        status,
    ...
    calculateFine: (borrowId) => {
        ...
        if (today <= dueDate)
            return 0;
        const diffTime = Math.ceil((today.getTime() - dueDate.getTime()) / (1000 * 60 * 60 * 24));
